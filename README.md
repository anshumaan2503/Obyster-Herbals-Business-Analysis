# 🌿 OYSTER HERBALS: Digital Commerce Platform
### 📋 End-to-End Business Analysis Portfolio Project

[![Project Status](https://img.shields.io/badge/Project%20Status-Completed-success?style=flat-square)](#)
[![Role](https://img.shields.io/badge/Role-Business%20Analyst-blue?style=flat-square)](#)
[![Methodology](https://img.shields.io/badge/Methodology-BPR%20%26%20Agile%20Scrum-orange?style=flat-square)](#)
[![Industry](https://img.shields.io/badge/Industry-Ayurvedic%20Wellness%20%2F%20Conversational%20Commerce-red?style=flat-square)](#)

---

## 🍃 Executive Summary

**Oyster Herbals and Panchkarma Centre** (located in Lal Diggi, Aligarh, Uttar Pradesh, India) is an established provider of traditional Ayurvedic clinical consultations, Panchkarma therapies, and proprietary herbal remedies. Historically, the clinic's retail operations operated under a physical, face-to-face dispensary model. While successful locally, this model limited regional reach, relied on repetitive phone inquiries, and suffered from operational inefficiencies like manual transcription errors and pricing communication gaps.

The **Oyster Herbals Digital Commerce Platform** is a web-based Product Catalog and Conversational Ordering solution. It transitions legacy, manual workflows into an optimized, digital-first model by routing customer purchase requests directly through prefilled WhatsApp API messages, centralizing catalog management, and introducing a verified reviews ecosystem.

Key operational improvements validated by this transformation include:
*   **67% reduction in order processing time** (from an average of **15 minutes down to below 5 minutes**).
*   **99% reduction in catalog update completion time** (from **up to 24 hours down to below 5 minutes** via the admin dashboard).
*   **87.5% reduction in order entry error rate** (from **8% of orders down to below 1%** via WhatsApp order details automation).
*   **100% elimination of pricing inconsistencies** (from **5% occurrence rate down to 0%** through a single source of truth).
*   **50% reduction in routine product inquiry calls** by providing a public, 24/7 self-service online catalog.
*   **30% targeted increase in out-of-town orders within six months** by expanding market reach across Uttar Pradesh.

This repository serves as a comprehensive **Business Analysis Portfolio**, demonstrating the end-to-end execution of the requirements engineering lifecycle, strategic assessment, business process improvement (BPI), Agile backlog planning, and interactive business intelligence.

---

## 📂 Portfolio Structure & Deliverables

This project is organized into modular directories containing professional deliverables mapping to industry-standard business analysis practices:
...
└── 📁 10_Power Bi                                   # Power BI Dashboard Reports & Datasets
    ...
    ├── 🖼️ Customer Insights Dashboard.png
    ├── 🖼️ Product Analytics Dashboard.png
    ├── 🖼️ Product Performance Dashboard.png
    └── 📂 Raw Datasets
        ├── 📊 Oyster_Herbals_Categories.xlsx
        ├── 📊 Oyster_Herbals_Products.xlsx
        └── 📊 Oyster_Herbals_Reviews.xlsx
```

> [!NOTE]
> All PDF portfolio deliverables were created and compiled using LaTeX to ensure professional-grade document design and typesetting.
...
*   **SWOT Analysis:** Documented Oyster Herbals' strategic position:
    *   *Strengths:* Established local clinical reputation, proprietary formulations.
    *   *Weaknesses:* Geographic dependency, manual sales processing.
...
<details>
<summary>🔍 View Sample Requirements Traceability Matrix (RTM)</summary>

| Business Req ID | Business Requirement | Target Customer Persona / Need | Feature / Use Case ID | UAT Scenario ID | QA Verification Description |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **BRD-FR-01** | Public Product Catalog | **Priya Patel** - Needs to browse authentic, doctor-formulated remedies. | UC-001: Browse Catalog | UAT-SC-001 | Verify product display is responsive and accurately renders prices, descriptions, and images. |
| **BRD-FR-02** | WhatsApp Ordering Routing | **Ramesh Sharma** - Wants to easily re-order regular remedies without travel. | UC-003: WhatsApp Order | UAT-SC-003 | Validate "Buy Now" routes to WhatsApp with correct phone number and prefilled order details. |
| **BRD-FR-03** | User Portal & Registration | **Arjun Mehta** - Wants to secure an account to verify purchase details. | UC-004: User Portal | UAT-SC-004, UAT-SC-005 | Verify secure account creation, duplicate registration rejection, and authentication validation. |
| **BRD-FR-04** | Verified Reviews Loop | **Arjun Mehta** - Relies heavily on social proof to buy natural remedies. | UC-006: Write Review | UAT-SC-006 | Validate that authenticated users can submit star ratings/reviews, preventing duplicates. |
| **BRD-FR-05** | Secure Admin Portal | **Clinic Administrative Staff** - Needs direct, self-service controls over the product listings. | UC-007: Manage Catalog | UAT-SC-007, UAT-SC-008 | Verify admin dashboard credentials and CRUD operations for adding, editing, or deleting products. |
...
*   **Business Gap Analysis Matrix:** Classified gaps into:
    ...
    *   *Process Efficiency Gap:* Manual order processing causing delays and transcription errors.
...
*   **UAT Defect Ledger:**
    *   *DFT-001 (Minor):* Inconsistent WhatsApp order text spacing (Closed).
    *   *DFT-002 (Medium):* Double review submission validation logic (Closed).
    *   *DFT-003 (High):* Admin dashboard routing authentication security (Closed).
    *   *DFT-004 (High):* Media pipeline fallback for cloud image upload delays (Closed).
*   **Business Sign-Off:** Formally approved by Dr. Anshumaan, Clinic Administration, and BA Anshuman Tiwari.

---

## 📊 10. `10_Power Bi` (Data Modeling & Analytics)

To establish data-driven business capabilities, a star-schema relational data model was designed representing the database of the commerce platform:
*   **Categories (10 categories):** Maps out the Ayurvedic inventory (Category ID, Category Name).
*   **Products (50 products):** Features proprietary remedies, prices in Indian Rupees (₹), active status, and descriptions.
*   **Reviews (500 reviews):** Tracks customer ratings (1–5 stars), customer names, dates, and review sentiment types (Positive, Neutral, Negative).

The Power BI model transforms these datasets into interactive executive dashboards to track customer sentiment, active product ratios, and catalog performance.

### 🖥️ Power BI Dashboard Previews

#### 📍 Customer Insights Dashboard
*Visualizes customer review ratings, rating distributions, sentiment analysis (positive/neutral/negative), and feedback trends over time.*
![Customer Insights Dashboard](10_Power%20Bi/Customer%20Insights%20Dashboard.png)

#### 📍 Product Analytics Dashboard
*Displays category-wise product distribution, product active status split, and price distributions/analyses across categories.*
![Product Analytics Dashboard](10_Power%20Bi/Product%20Analytics%20Dashboard.png)

#### 📍 Product Performance Dashboard
*Synthesizes product sales velocity, average ratings per product line, and performance metrics cross-referenced with categories.*
![Product Performance Dashboard](10_Power%20Bi/Product%20Performance%20Dashboard.png)

---

## 💻 Tech Stack & Tools Applied

*   **Process Modeling & Swimlanes:** **Draw.io** (BPMN 2.0 notation)
*   **UI/UX Wireframing:** **Figma** (Interactive layouts and product mockups)
*   **Agile & Backlog Management:** **Jira Software** (Feature backlog planning)
*   **Data Modeling & Analytics:** **Power BI Desktop** (DAX formulas, Star Schema modeling, Visualizations)
*   **Requirements & Traceability:** **MS Excel** (RTM construction, data cataloging)
*   **Document Typesetting:** **LaTeX** (High-quality PDF compile, typography styling)
...
*   **Project:** Oyster Herbals Digital Commerce Platform
*   **Date:** June 2026
